...
export async function buildTreasuryPdf({ draft, status = 'draft', preparedBy = '', preparerRole = '', preparerSignature }) {
  ...
  newPage();paragraph(draft.previousMeetingDate ? `Reporting window: posted activity after ${date(draft.previousMeetingDate)} through ${date(draft.periodEnd)}` : `Period: ${date(draft.periodStart)} through ${date(draft.periodEnd)}`); if(draft.presentedOn)paragraph(`Date presented: ${date(draft.presentedOn)}`);if(draft.bankName)paragraph(draft.bankName);
