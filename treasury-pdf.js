...
const date = value => value ? new Intl.DateTimeFormat('en-US', { weekday:'long',month:'long',day:'numeric',year:'numeric',timeZone:'UTC' }).format(new Date(`${value}T00:00:00Z`)) : 'Not provided';
const shortDate = value => value ? new Intl.DateTimeFormat('en-US', { month:'numeric',day:'numeric',year:'numeric',timeZone:'UTC' }).format(new Date(`${value}T00:00:00Z`)) : '';
const titleDate = value => value ? new Intl.DateTimeFormat('en-US', { month:'long',day:'numeric',year:'numeric',timeZone:'UTC' }).format(new Date(`${value}T00:00:00Z`)) : 'date not provided';
const pdfCurrency = cents => cents === null || cents === undefined ? 'Not provided' : currency(cents);
...
export async function buildTreasuryPdf({ draft, status = 'draft', preparedBy = '', preparerRole = '', preparerSignature }) {
  ...
  const importedValues=calc.accounts.flatMap(account=>[
    ['Beginning Balance',account.opening],['Total Receipts',account.receipts],['Total Disbursements',account.disbursements],
    ['Transfers In',account.transfersIn],['Transfers Out',account.transfersOut],['Statement Ending Balance',account.statement],
    ['Deposits in Transit',account.transit],['Outstanding Checks',account.outstanding],['Book Balance',account.book],
  ].filter(([,value])=>value!==null&&value!==undefined).map(([label,value])=>({account:account.name,label,value})));
  if(importedValues.length){
    heading('Values Organized from the Uploaded Records');
    let accountName='';
    for(const item of importedValues){
      if(item.account!==accountName){room(31);page.drawText(clean(item.account),{x:48,y,size:10,font:bold,color:navy});y-=17;accountName=item.account;}
      row(item.label,pdfCurrency(item.value));
    }
  }
  ...
  for(const account of calc.accounts) {
    ...
    heading('Receipts');
    if(receipts.length)for(const item of receipts)row(item.description||'Receipt',pdfCurrency(money(item.amount)));
    else paragraph('No receipts were identified in the uploaded records for this reporting period.');
    row('Total Receipts',pdfCurrency(account.receipts),true);
    heading('Disbursements');
    if(disbursements.length)for(const item of disbursements)row(`${item.description||'Disbursement'}${item.reference?` (Check ${item.reference})`:''}`,pdfCurrency(money(item.amount)));
    else paragraph('No disbursements were identified in the uploaded records for this reporting period.');
    row('Total Disbursements',pdfCurrency(account.disbursements),true);
    if(transfersIn.length||transfersOut.length){
      heading('Transfers');
      for(const item of transfersIn)row(`Transfer In: ${item.description||'Account transfer'}`,pdfCurrency(money(item.amount)));
      for(const item of transfersOut)row(`Transfer Out: ${item.description||'Account transfer'}`,pdfCurrency(money(item.amount)));
    }
    heading('Monthly Summary');
    row('Beginning Balance',pdfCurrency(account.opening));row('Plus: Total Receipts',pdfCurrency(account.receipts));row('Less: Total Disbursements',pdfCurrency(account.disbursements));
    if(account.transfersIn!==null&&account.transfersIn!==0)row('Plus: Transfers In',pdfCurrency(account.transfersIn));
    if(account.transfersOut!==null&&account.transfersOut!==0)row('Less: Transfers Out',pdfCurrency(account.transfersOut));
    row('Ending Bank Balance',pdfCurrency(account.calculated),true);
    heading('Account Reconciliation');
    row('Bank Statement Ending Balance',pdfCurrency(account.statement));row('Plus: Deposits in Transit',pdfCurrency(account.transit));row('Less: Outstanding Checks',pdfCurrency(account.outstanding));row('Adjusted Bank Balance',pdfCurrency(account.adjusted),true);row("Treasurer's Book Balance",pdfCurrency(account.book));row('Difference, if any',pdfCurrency(account.difference),true);
  }

  room(105+Math.min(draft.obligations.length,6)*24);heading('Outstanding Obligations');
  if(!draft.obligations.length)paragraph(draft.obligationsReviewed?'No outstanding obligations reported.':'No outstanding obligations were identified in the uploaded records.');
  for(const item of draft.obligations)row(`${item.name}${item.dueDate?` (Due ${titleDate(item.dueDate)})`:''}${item.note?` - ${item.note}`:''}`,pdfCurrency(money(item.amount)));
  if(draft.obligations.length)row('Total Outstanding Obligations',pdfCurrency(calc.obligations),true);

  room(105+Math.min(draft.funds.length,6)*24);heading('Fenced Money');
  if(!draft.funds.length)paragraph(draft.fundsReviewed?'No fenced money reported.':'No fenced money was identified in the uploaded records.');
  for(const fund of draft.funds)row(`${fund.name}${fund.restriction?` - ${fund.restriction}`:''}`,pdfCurrency(money(fund.amount)));
  if(draft.funds.length)row('Total Fenced Money',pdfCurrency(calc.fenced),true);
