...
export async function buildTreasuryPdf({ draft, status = 'draft', preparedBy = '', preparerRole = '', preparerSignature }) {
  ...
  newPage();paragraph(`Reporting period: ${date(draft.periodStart)} through ${date(draft.periodEnd)}`); if(draft.presentedOn)paragraph(`Date presented: ${date(draft.presentedOn)}`);if(draft.bankName)paragraph(draft.bankName);
