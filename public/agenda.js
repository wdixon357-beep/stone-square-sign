...
export class AgendaWorkspace {
  ...
  async run(action, button) {
    if (this.busy) return;
    try {
      if (action === 'new') { this.busy = true; const { agenda } = await this.api('/api/agendas', { method: 'POST', body: JSON.stringify({}) }); this.open(agenda); this.message('New agenda created.'); return; }
      ...
      if (action === 'add-section') { this.draft.sections.push({ id: crypto.randomUUID(), heading: 'New Agenda Section', scheduledTime: '', body: '' }); this.dirty = true; this.renderEditor(); this.schedule(); return; }
      if (action === 'remove-section') { this.draft.sections.splice(Number(button.dataset.index), 1); this.dirty = true; this.renderEditor(); this.schedule(); return; }
      if (action === 'move-up' || action === 'move-down') {
        ...
        this.dirty = true; this.renderEditor(); this.schedule(); return;
      }
      this.busy = true; button.disabled = true;
      if (action === 'save') { await this.save(); this.message('Agenda saved.'); }
      if (action === 'delete') {
  ...
  async list() {
    ...
    this.root.innerHTML = `<div class="content-head"><div><p class="eyebrow">WORSHIPFUL MASTER</p><h1>Agenda Creator</h1><p>Create the meeting order, review the exact document, and download the PDF.</p></div><button class="primary" data-agenda="new" type="button">Create agenda</button></div><p id="agendaMessage" class="message" role="status"></p><div id="agendaList" class="agenda-list"></div>`;
    try {
      this.records = (await this.api('/api/agendas')).agendas;
      this.root.querySelector('#agendaList').innerHTML = this.records.length ? this.records.map(item => `<article class="agenda-record"><div><h2>${esc(this.dateLabel(item.draft.meetingDate))}</h2><p>${esc(item.draft.meetingType)} · Updated ${esc(new Date(item.updatedAt).toLocaleString())}</p></div><div class="row-buttons"><button class="secondary" data-agenda="open" data-id="${esc(item.id)}">Open</button><button class="secondary danger-text" data-agenda="delete" data-id="${esc(item.id)}" data-revision="${item.revision}">Delete</button></div></article>`).join('') : '<div class="empty-state"><h2>No agendas yet</h2><p>Create an agenda to begin with the Lodge format.</p></div>';
    } catch (error) { this.message(error.message, true); }
  }
  dateLabel(value) {
  ...
  open(record) { this.record = record; this.draft = clone(record.draft); this.dirty = false; this.renderEditor(); this.schedule(); }
