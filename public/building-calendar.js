...
export const canAttestBuildingAgreement = (request, user) => Boolean(request && ['secretary', 'assistant_secretary'].includes(user?.role) && (Array.isArray(request.attestationRoles) ? request.attestationRoles : ['secretary']).includes(user.role));
export const buildingAttestationStatus = request => (request?.attestationRoles || ['secretary']).includes('assistant_secretary') ? 'Awaiting Secretary or Assistant Secretary attestation' : 'Awaiting Secretary attestation';
export const buildingSignerTitle = user => user?.role === 'assistant_secretary' ? 'Assistant Secretary' : 'Secretary';
...
export class BuildingCalendarWorkspace {
  ...
  async building(){
    ...
    try{const result=await this.api('/api/building/requests');if(sequence!==this.buildingSequence)return;this.requests=result.requests;this.mayDecide=result.canDecide;
      this.buildingRoot.querySelector('.building-requests').innerHTML=this.requests.length?this.requests.map(request=>`<article class="panel building-request"><div class="content-head"><div><h2>${escape(request.organization)}</h2><p>${escape(request.date)} · ${escape(request.start||'Time not provided')}${request.end?' to '+escape(request.end):''}</p></div><span class="status">${escape(request.agreementStatus==='awaiting_secretary_attestation'?buildingAttestationStatus(request):request.agreementStatus==='fully_executed'?'Agreement complete':request.status)}</span></div><p>${escape(request.spaces?.join(', '))}</p>${frontYardOnly(request.spaces)?`<p><strong>Restroom access:</strong> ${request.bathroomAccess?'Yes':'No'}</p>`:''}<p>${escape(request.description)}</p><p>${escape(request.contactName)}${request.contact?' · '+escape(request.contact):''}</p>${request.coordinator?`<p>Request coordinator: ${escape(request.coordinator.name)} · ${escape(request.coordinator.email)}</p>`:''}${request.agreementText?`<details><summary>View full Building Use Agreement</summary><div class="building-agreement-text">${escape(request.agreementText)}</div></details>`:''}<p>Reference: ${escape(request.id)}</p>${request.note?`<p>Decision note: ${escape(request.note)}</p>`:''}${request.decidedBy?`<p>Recorded by ${escape(request.decidedBy)}${request.decidedAt?' · '+escape(request.decidedAt):''}</p>`:''}${request.attestedBy?`<p>Attested by ${escape(request.attestedBy.name)}${request.attestedBy.title?', '+escape(request.attestedBy.title):''}</p>`:''}${request.status!=='pending'?`<p>${request.requesterNotified?'Requester notification recorded.':'Requester notification will be sent after all required signatures are recorded.'}</p>`:''}${request.agreementStatus==='awaiting_secretary_attestation'&&canAttestBuildingAgreement(request,this.user())?`<div data-building-attestation><p class="helper">Review the approved agreement before applying your own saved ${escape(buildingSignerTitle(this.user()))} signature.</p><button class="primary" data-building="attest" data-id="${escape(request.id)}">Attest agreement</button></div>`:this.can('building.decide')&&result.canDecide&&request.status==='pending'?`<div data-building-controls><label for="building-note-${escape(request.id)}">Note to the requester</label><textarea id="building-note-${escape(request.id)}" maxlength="2000">${escape(request.note||'')}</textarea>${request.ownerOnly?`<fieldset><legend>Lodge authorization and final conditions</legend><label>Authorization date<input id="building-auth-date-${escape(request.id)}" type="date"></label><label>Minutes or resolution reference<input id="building-auth-record-${escape(request.id)}" maxlength="500"></label><label>Approved fee<input id="building-auth-fee-${escape(request.id)}" maxlength="100" placeholder="Example: $25 per approved session"></label><label>Insurance decision<select id="building-auth-insurance-${escape(request.id)}"><option value="">Choose</option><option value="required">Required</option><option value="waived">Waived</option></select></label><p>No security deposit is required.</p><label>Conditions<textarea id="building-auth-conditions-${escape(request.id)}" maxlength="2000"></textarea></label></fieldset><p class="helper">Worshipful Master signature: approval applies your saved signature. The officer designated in this agreement must attest before the organization receives payment access.</p>`:'<p class="helper">The portal sends the decision notification to the requester and officers.</p>'}<div class="row-buttons"><button class="primary" data-building="approved" data-id="${escape(request.id)}">Approve</button><button class="secondary" data-building="denied" data-id="${escape(request.id)}">Decline</button></div></div>`:''}</article>`).join(''):'<p>No building requests.</p>';
      [...this.buildingRoot.querySelectorAll('.building-request')].forEach((card,index)=>{
        const request=this.requests[index];card.dataset.requestId=request.id;card.tabIndex=-1;
        if(request.statusOnly){
          card.innerHTML=`<div class="content-head"><div><h2>${escape(request.organization)}</h2><p>${escape(request.date)}</p></div><span class="status">Awaiting Secretary attestation</span></div><p>This signed version names Secretary McDuffie as the attesting officer. You can follow its status here. The organization would need to accept a revised agreement before the Assistant Secretary could sign.</p><p>Reference: ${escape(request.id)}</p>`;
        }else if(request.filedCompletedAt){
          card.querySelector('.status').textContent='Filed as completed';
          const filed=document.createElement('p');filed.textContent=`Filed by ${request.filedCompletedBy||'the Worshipful Master'} on ${new Date(request.filedCompletedAt).toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}.`;card.append(filed);
        }else if(this.user()?.role==='owner'&&request.status==='approved'&&request.agreementStatus==='fully_executed'&&request.attestedBy?.at){
          const action=document.createElement('div');action.className='row-buttons';action.innerHTML=`<button class="primary" data-building="file-completed" data-id="${escape(request.id)}">File as completed</button>`;card.append(action);
        }
      });
  ...
  async decision(button){
    ...
    if(decision==='file-completed'){
      if(this.buildingBusy||this.user()?.role!=='owner'||request.status!=='approved'||request.agreementStatus!=='fully_executed'||!request.attestedBy?.at||request.filedCompletedAt)return;
      if(!window.confirm(`File the completed Building Use Agreement for ${request.organization} on ${request.date}? This records the Lodge's administrative filing after both signatures.`))return;
      this.buildingBusy=true;button.disabled=true;
      try{await this.api(`/api/building/requests/${encodeURIComponent(request.id)}/file-completed`,{method:'POST',body:JSON.stringify({revision:request.revision})});await this.building();this.message(this.buildingRoot,'The signed agreement is filed as completed.');window.dispatchEvent(new Event('buildingWorkflowChanged'));}
      catch(error){if(error.status===409)await this.building();this.message(this.buildingRoot,error.message||'The completed agreement could not be filed.');}
      finally{this.buildingBusy=false;if(button.isConnected)button.disabled=false;}return;
    }
    if(decision==='attest'){
      if(this.buildingBusy||!canAttestBuildingAgreement(request,this.user())||request.status!=='approved'||request.agreementStatus!=='awaiting_secretary_attestation')return;
      if(!window.confirm(`Attest the approved Building Use Agreement for ${request.organization} with your own saved ${buildingSignerTitle(this.user())} signature?`))return;
      this.buildingBusy=true;button.disabled=true;try{const result=await this.api(`/api/building/requests/${encodeURIComponent(request.id)}/attest`,{method:'POST',body:JSON.stringify({revision:request.revision})});await this.building();this.message(this.buildingRoot,[result.request?.requesterNotified?'Attestation recorded. The organization was notified and payment access is available.':"Attestation recorded. The organization's notification has not been confirmed.",...(result.notificationWarnings||[])].join(' '));window.dispatchEvent(new Event('buildingWorkflowChanged'));}catch(error){this.message(this.buildingRoot,error.message||'The attestation could not be recorded.');}finally{this.buildingBusy=false;if(button.isConnected)button.disabled=false;}return;
    }
    ...
    let authorization;
    if(decision==='approved'&&request.ownerOnly){authorization={date:document.getElementById('building-auth-date-'+request.id)?.value||'',record:document.getElementById('building-auth-record-'+request.id)?.value||'',fee:document.getElementById('building-auth-fee-'+request.id)?.value||'',insurance:document.getElementById('building-auth-insurance-'+request.id)?.value||'',conditions:document.getElementById('building-auth-conditions-'+request.id)?.value||''};if(!authorization.date||!authorization.record.trim()||!authorization.fee.trim()||!authorization.insurance){this.message(this.buildingRoot,'Complete the Lodge authorization, final fee, and insurance decision before approval.');return;}}
    ...
    this.buildingBusy=true;button.disabled=true;
    try{const result=await this.api(`/api/building/requests/${encodeURIComponent(request.id)}/decision`,{method:'POST',body:JSON.stringify({decision,note,revision:request.revision,authorization})});await this.building();this.message(this.buildingRoot,[decision==='approved'&&request.ownerOnly?'Approval and Worshipful Master signature recorded. The designated officer must still attest.':'Decision recorded. Check the request card for notification status.',...(result.notificationWarnings||[])].join(' '));window.dispatchEvent(new Event('buildingWorkflowChanged'));}
  ...
  refreshPermissions(){this.buildingRoot.querySelectorAll('#buildingRequestForm,[data-building="request-new"]').forEach(element=>element.hidden=!this.can('building.request'));const queue=this.buildingRoot.querySelector('.building-requests');if(queue)queue.hidden=!this.can('building.view');this.buildingRoot.querySelectorAll('[data-building-controls]').forEach(element=>element.hidden=!this.can('building.decide'));this.buildingRoot.querySelectorAll('[data-building-attestation]').forEach(element=>{const request=this.requests?.find(item=>item.id===element.querySelector('[data-id]')?.dataset.id);element.hidden=!canAttestBuildingAgreement(request,this.user());});this.calendarRoot.querySelectorAll('[data-calendar-manage],#calendarEventForm').forEach(element=>element.hidden=!this.can('calendar.manage'));}
}
