...
struct ActivityPerson: Decodable, Identifiable {var id:Int;var name:String;var role:String;var rosterLinked:Bool;var revoked:Bool}
...
@MainActor final class ActivityPresence: ObservableObject {
 private weak var model:AppModel?;private var timer:Timer?;private var eventMonitor:Any?;private var lastInteraction=Date();private var lastActivationSent=Date.distantPast;private var lastScrollSent=Date.distantPast;private var busy=false
 var area="home"
 func start(_ model:AppModel){stop();self.model=model;lastInteraction=Date();eventMonitor=NSEvent.addLocalMonitorForEvents(matching:[.keyDown,.leftMouseDown,.rightMouseDown,.scrollWheel]){[weak self] event in
  guard let self else{return event};self.lastInteraction=Date()
  if event.type == .scrollWheel, Date().timeIntervalSince(self.lastScrollSent) >= 15 {self.lastScrollSent=Date();Task{@MainActor in await self.interaction("scroll",target:"")}}
  else if [.leftMouseDown,.rightMouseDown].contains(event.type),Date().timeIntervalSince(self.lastActivationSent) >= 1 {self.lastActivationSent=Date();Task{@MainActor in await self.interaction("activate",target:"Mac app control")}}
  return event
 };timer=Timer.scheduledTimer(withTimeInterval:30,repeats:true){[weak self] _ in Task{@MainActor in await self?.pulse()}};Task{await pulse()}}
 func visit(_ section:AppSection?){area=section.map{String(describing:$0)} ?? "home";Task{await pulse()}}
 func interaction(_ kind:String,target:String) async {guard let model,model.user != nil else{return};let body=try? JSONSerialization.data(withJSONObject:["kind":kind,"area":area,"target":target]);let _:EmptyResponse?=try? await model.request("/api/activity/interaction",method:"POST",body:body)}
...
struct OfficerActivityView:View {
 ...
 var body: some View {
  VStack(spacing: 0) {
   NativeWorkspaceHeader(title: "Dashboard Activity", subtitle: "Account sign-ins, areas visited, interactions and estimated active time", symbol: "clock.arrow.circlepath") {
    Button("Refresh", systemImage: "arrow.clockwise") { Task { await workspace.load() } }
   }
   HStack {
    Picker("Account", selection: $workspace.person) { Text("All accounts").tag(0); ForEach(workspace.data?.users ?? []) { user in Text(user.name + (user.revoked ? " (access revoked)" : "")).tag(user.id) } }
   ...
   List {
     ...
     Section("Sign-ins and time") {
      ForEach(workspace.data?.sessions ?? []) { entry in
       VStack(alignment: .leading, spacing: 6) {
        ...
        Text("\(entry.client)" + (entry.area.isEmpty ? "" : " · Last area: \(entry.area)")).font(.callout)
        ...
        if !entry.areas.isEmpty { LabeledContent("Time by area") { VStack(alignment:.trailing,spacing:2) { ForEach(Array(entry.areas.enumerated()),id:\.offset) { _, item in Text("\(item.area): \(activeDuration(item.seconds))") } } } }
    ...
    Section {
     DisclosureGroup("About activity measurements") {
      Text("Times are Eastern. The filter selects sign-ins seen during the chosen period; each time total covers that full recorded sign-in. Active time is estimated from interaction within the past two minutes while the app is in the foreground. Idle, background and disconnected intervals do not count. Interaction records identify dashboard controls and never include passwords, form entries, banking source content, or private suggestion text. Older actions may have no timing information.").font(.caption).foregroundStyle(.secondary)
