...
function setup(){
 const elements={},calls=[],state={token:'session-a',user:{id:2,role:'officer',permissions:['minutes.prepare']},activeSection:'minutes'};
 ...
 const ctx={state,$:id=>elements[id]||(elements[id]={}),window:{setInterval:(fn,ms)=>calls.push(['interval',ms]),addEventListener:(event)=>calls.push(['event',event])},document:{querySelectorAll:()=>[]},apiFetch:async()=>result,can:key=>state.user.permissions.includes(key),applyWorkspacePermissions:user=>calls.push(['apply',user]),showWorkspaceSection:(...args)=>calls.push(['section',...args]),treasuryWorkspace:{refreshPermissions:()=>calls.push(['treasury'])},hide:el=>{el.hidden=true},show:el=>{el.hidden=false},renderDocuments:()=>calls.push(['documents']),setMessage:()=>{},authMessage:{}};
...
t=setup();let complete;t.ctx.apiFetch=()=>new Promise(resolve=>complete=resolve);const pending=t.refresh();t.state.token='different-session';complete({user:{id:2,role:'officer',permissions:[]}});await pending;assert.equal(t.state.user.permissions[0],'minutes.prepare');assert.equal(t.calls.filter(x=>x[0]==='apply').length,0);
