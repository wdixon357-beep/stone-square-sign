...
import { BuildingCalendarWorkspace, monthBounds, eventOccurs, formatCalendarTime, eventTime, validateBuildingRequest, canAttestBuildingAgreement, buildingAttestationStatus } from '../public/building-calendar.js';
...
assert.equal(canAttestBuildingAgreement(secretaryRequest,{role:'secretary'}),true);
assert.equal(canAttestBuildingAgreement(secretaryRequest,{role:'assistant_secretary'}),false);
assert.equal(buildingAttestationStatus(secretaryRequest),'Awaiting Secretary attestation');
const eitherOfficerRequest={...secretaryRequest,attestationRoles:['secretary','assistant_secretary']};
assert.equal(canAttestBuildingAgreement(eitherOfficerRequest,{role:'assistant_secretary'}),true);
assert.equal(canAttestBuildingAgreement(eitherOfficerRequest,{role:'warden'}),false);
assert.equal(canAttestBuildingAgreement({...eitherOfficerRequest,attestationRoles:['warden']},{role:'warden'}),false);
assert.equal(buildingAttestationStatus(eitherOfficerRequest),'Awaiting Secretary or Assistant Secretary attestation');
...
const assistantWorkspace=new BuildingCalendarWorkspace({user:()=>({role:'assistant_secretary',permissions:['building.view']}),api:async(path,init)=>{assistantCalls.push({path,init});return init?{request:eitherOfficerRequest}:{requests:[eitherOfficerRequest],canDecide:false};}});
await assistantWorkspace.building();
assert.match(assistantWorkspace.buildingRoot.querySelector('.building-requests').innerHTML,/data-building="attest"/);
await assistantWorkspace.decision({dataset:{building:'attest',id:eitherOfficerRequest.id}});
assert.equal(assistantCalls[1].path,'/api/building/requests/SSL-TEST/attest');
assistantWorkspace.requests=[secretaryRequest];
await assistantWorkspace.decision({dataset:{building:'attest',id:secretaryRequest.id}});
assert.equal(assistantCalls.length,3);
const completedRequest={...eitherOfficerRequest,agreementStatus:'fully_executed',attestedBy:{name:'Adrian Reese',title:'Assistant Secretary',at:'2026-09-23T12:00:00Z'}};
const completedWorkspace=new BuildingCalendarWorkspace({user:()=>({role:'owner',permissions:['building.view']}),api:async()=>({requests:[completedRequest],canDecide:false})});
completedWorkspace.requestFormOpen=true;
await completedWorkspace.building();
assert.match(completedWorkspace.buildingRoot.querySelector('.building-requests').innerHTML,/Attested by Adrian Reese, Assistant Secretary/);
