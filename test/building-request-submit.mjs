...
try{
 ...
 check('Front-yard-only request requires restroom choice',(await api(path,requester,'POST',{...valid,submissionId:crypto.randomUUID(),spaces:['Front yard']})).status===400);
 const frontYard=await api(path,requester,'POST',{...valid,submissionId:crypto.randomUUID(),spaces:['Front yard'],bathroomAccess:false});check('Front-yard-only restroom choice is forwarded',frontYard.status===201&&submissions.at(-1).bathroomAccess===false);
