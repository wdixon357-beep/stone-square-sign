...
const {easternDate}=await import('../treasury-period.js');
...
try{
  ...
  const oldWindow={...structuredClone(completeTreasuryFixture),periodStart:'2098-12-01',periodEnd:'2099-01-01'};
  await dbRun('INSERT INTO treasury_reports(id,draft_json,source_text,status,created_by_user_id,preparer_name,preparer_role,created_at,updated_at,preparer_user_id,uploader_name) VALUES(?,?,?,?,?,?,?,?,?,?,?)',['old-window-draft',JSON.stringify(oldWindow),'Synthetic old-window source','draft',preparer,'Preparer','treasury_preparer',time,time,preparer,'Preparer']);
  await dbRun("DELETE FROM treasury_migrations WHERE key='rolling_report_window_2026_09_15'");
  await initTreasurySchema();
  const migrated=await dbGet("SELECT draft_json,revision FROM treasury_reports WHERE id='old-window-draft'");
  const migratedDraft=JSON.parse(migrated.draft_json);
  check('Former future-ending drafts migrate once to the preparation date',migratedDraft.periodEnd===easternDate());
  await initTreasurySchema();
  check('The corrected reporting window remains frozen after another startup',(await dbGet("SELECT revision FROM treasury_reports WHERE id='old-window-draft'")).revision===migrated.revision);
