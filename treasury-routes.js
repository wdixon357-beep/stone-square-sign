...
const editable = (row,user) => TREASURY_ROLES.has(user.role)&&row.status==='draft'&&(row.preparer_user_id===user.id||user.role==='owner');
export async function treasuryAccess(user) {
  if(TREASURY_ROLES.has(user.role))return 'prepare';
  return await dbGet('SELECT 1 FROM treasury_upload_access WHERE user_id=?',[user.id])?'upload':null;
}
...
export function mountTreasuryRoutes(app,{requireAuth,rateLimit,sendEmail,baseUrl,broadcast,generationFor=()=>undefined}) {
  ...
  const record=async req=>{const row=await fetchRecord(req.params.id);if(!row||req.treasuryAccess==='upload'&&row.created_by_user_id!==req.user.id)throw error(404,'Treasurer report not found.');return row;};
  ...
  app.get('/api/treasury/access',route(async(req,res)=>{
    ...
    const users=await dbAll("SELECT u.id,u.name,u.role,(g.user_id IS NOT NULL) AS upload_enabled FROM users u LEFT JOIN treasury_upload_access g ON g.user_id=u.id WHERE u.access_revoked_at IS NULL AND u.email NOT LIKE '%.local' ORDER BY u.name");
    res.json({users:users.map(u=>({id:u.id,name:u.name,role:u.role,canPrepare:TREASURY_ROLES.has(u.role),uploadEnabled:u.upload_enabled}))});
  }));
  app.put('/api/treasury/access/:userId',route(async(req,res)=>{
    if(req.user.role!=='owner')throw error(403,'Only the administrator can manage bank record access.');
    const user=await dbGet('SELECT id,role FROM users WHERE id=? AND access_revoked_at IS NULL',[Number(req.params.userId)]);
    if(!user||typeof req.body.enabled!=='boolean')throw error(400,'Choose an active account and its bank record access.');
    if(TREASURY_ROLES.has(user.role))throw error(409,'This officer already has access through the assigned office.');
  ...
  app.get('/api/treasury/preparers',route(async(_req,res)=>{
    const users=await dbAll("SELECT id,name,role FROM users WHERE access_revoked_at IS NULL AND email NOT LIKE '%.local' ORDER BY name");res.json({preparers:users.filter(u=>TREASURY_ROLES.has(u.role))});
  }));
  app.get('/api/treasury',route(async(req,res)=>res.json({reports:(await dbAll('SELECT * FROM treasury_reports WHERE deleted_at IS NULL'+(req.treasuryAccess==='upload'?' AND created_by_user_id=?':'')+' ORDER BY created_at DESC',req.treasuryAccess==='upload'?[req.user.id]:[])).map(serial)})));
  app.post('/api/treasury/generate',rateLimit({key:'treasury-generate',maximum:12,windowMs:3600000}),upload.array('files',5),route(async(req,res)=>{
    ...
    res.status(201).json({report:serial(await fetchRecord(id))});
  }));
  app.post('/api/treasury/:id/assign',route(async(req,res)=>{
    ...
    const preparer=await dbGet("SELECT id,name,role FROM users WHERE id=? AND access_revoked_at IS NULL AND email NOT LIKE '%.local'",[Number(req.body.preparerUserId)]);
    if(!preparer||!TREASURY_ROLES.has(preparer.role))throw error(400,'Choose an active officer with treasurer report preparation access.');
  ...
  app.get('/api/treasury/:id/sources/:sourceId',route(async(req,res)=>{
  ...
  app.put('/api/treasury/:id',route(async(req,res)=>{
  ...
  app.post('/api/treasury/:id/preview',route(async(req,res)=>{
    ...
    const signatures=await dbAll('SELECT phase,signature_bytes FROM treasury_attestations WHERE report_id=? ORDER BY created_at DESC',[row.id]);
    const pdf=await buildTreasuryPdf({draft,status:row.status,preparedBy:row.preparer_name,preparerRole:row.preparer_role,preparerSignature:bytes(signatures.find(s=>s.phase==='preparer')?.signature_bytes)});
    res.type('application/pdf').send(pdf);
  }));
  app.post('/api/treasury/:id/preparer-attest',route(async(req,res)=>{
    const row=await record(req);revision(req,row);
  ...
  app.post('/api/treasury/:id/mark-distributed',route(async(req,res)=>{
  ...
  app.delete('/api/treasury/:id',route(async(req,res)=>{
